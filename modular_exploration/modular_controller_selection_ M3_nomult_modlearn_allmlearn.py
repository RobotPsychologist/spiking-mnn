...
with model:
    ...
    def out1_func(t):
        ...
        if t % 10 < 3.3:
            return np.abs(np.sin(t*2*np.pi*2))
        elif t % 10 > 3.3 and t % 10 < 6.6:
            return 0
        else:
            return 0
    ...
    output_node = nengo.Ensemble(n_neurons=100, dimensions=1, radius=5, label='output_node') # NETWORK OUTPUT
    ...
    controller_m1_conn = nengo.Connection(controller[0], m1.neurons, transform=-10*np.ones([100,1])) 
    controller_m2_conn = nengo.Connection(controller[1], m2.neurons, transform=-10*np.ones([100,1])) 
    controller_m3_conn = nengo.Connection(controller[2], m3.neurons, transform=-10*np.ones([100,1])) 
    controller_m1error_conn = nengo.Connection(controller[0], error_mod1_out.neurons, transform=-10*np.ones([100,1])) 
    controller_m2error_conn = nengo.Connection(controller[1], error_mod2_out.neurons, transform=-10*np.ones([100,1])) 
    controller_m3error_conn = nengo.Connection(controller[2], error_mod3_out.neurons, transform=-10*np.ones([100,1])) 

    ## OUTPUTS
    m1_output = nengo.Connection(m1, output_node,synapse=prb_syn,function=mod_init_func, learning_rule_type=nengo.PES(learning_rate=1e-03)) 
    m2_output = nengo.Connection(m2, output_node,synapse=prb_syn,function=mod_init_func, learning_rule_type=nengo.PES(learning_rate=1e-03)) 
    m3_output = nengo.Connection(m3, output_node,synapse=prb_syn,function=mod_init_func, learning_rule_type=nengo.PES(learning_rate=1e-03)) 
    ...
    nengo.Connection(m1, error_mod1_out, transform=1)
    nengo.Connection(m2, error_mod2_out, transform=1)
    nengo.Connection(m3, error_mod3_out, transform=1)
    nengo.Connection(error_mod1_out, m1_output.learning_rule)
    nengo.Connection(error_mod2_out, m2_output.learning_rule)
    nengo.Connection(error_mod3_out, m3_output.learning_rule)