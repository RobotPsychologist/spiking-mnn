...
with model:
    ...
    ldn = LMUNetwork(n_neurons=n_neurons, theta=theta, q=q, size_in=2, tau=tau)
    ...
    nengo.Connection(stim, ldn.input[0], synapse=None )
    nengo.Connection(stim, ldn.input[1], synapse=None )
    ldn_cont_conn1 = nengo.Connection(ldn.output[0], controller[0],synapse=None, learning_rule_type=nengo.PES() )
    ldn_cont_conn2 = nengo.Connection(ldn.lmu[1], controller[1],synapse=None, learning_rule_type=nengo.PES() )
    ...
    ## Error Connections
    nengo.Connection(error_controller[0], ldn_cont_conn1.learning_rule)
    nengo.Connection(error_controller[1], ldn_cont_conn2.learning_rule)
